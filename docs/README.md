...
設計に対応する検証実装を `apps/` と `packages/` に置いている。`apps/` は auth-api / auth-web / crm-web / crm-api / cms-web / cms-api。web は React Router v8 の SPA と薄い BFF で、BFF は `packages/web-core`、共通の React コードは `packages/web-ui` に共有し、crm-web / cms-web は `src/main.ts` で起動関数を呼び、`app/features/` にサービス固有の画面を feature 単位で持つ。UI 部品は shadcn/ui、スタイルは Tailwind CSS v4、フォームは react-hook-form と `packages/api-contract` の入力スキーマで組む。判断事項D20。auth の画面も auth-web の SPA で、auth-api が同一オリジンで配り、SPA 向けの `/api/login` `/api/portal` `/api/logout` `/api/sessions` を持つ。資格情報は HTML フォームの POST で送る。判断事項D19。SSO Session は identity DB の `auth_sessions` に IP と User-Agent とともに記録し、ログイン、`/authorize` の到達、失効、再利用検知、招待と解除を `audit_events` に残す。本人はポータルの `/security` で他の端末を失効でき、招待の解除はそのサービスの Refresh Token 系列を即時に失効させる。揮発ストアのキーは秘密値の SHA-256。判断事項D21。api は `packages/api-core` をフレームワークとして使い、crm-api / cms-api が `definition.ts` で役割と権限を宣言し、エンドユーザーと投稿の feature を domain / application / infrastructure / interface の 4 層で持つ。管理アカウントの招待と権限編集の API はどのサービスにも api-core が付ける。リクエストとレスポンスの zod スキーマは `packages/api-contract` に置き、サーバーと SPA で共有する。起動方法と確認手順はリポジトリ直下の [README.md](../README.md) を参照する。MFA は全員必須で、認証アプリの TOTP のチャレンジと登録を auth-api と auth-web の `/login/challenge` `/login/mfa-setup` で実装済み。Cognito は OPTIONAL にし必須化は auth-api が行い、QR は 3 分で失効して再発行する。判断事項D22。Global Logout は Back-Channel Logout まで実装済み。`*-web` の画面はホーム、エンドユーザー、投稿、管理アカウントの招待と権限編集まで SPA で実装済み。判断事項D18。
