...
| 2 | 既存サービスに `/auth/login` `/auth/callback` `/auth/logout` `/auth/backchannel-logout` と新セッション Cookie を追加する。middleware は「新セッション Cookie があれば通す、なければ従来どおり判定する」の二重判定にする。画面のリンクと旧サインインはそのまま | 旧と新 |
| 3 | バックエンドの Token 検証を「従来の Cognito JWT でも Auth Server の JWT でも通す」にする。BFF は新セッションなら Bearer、旧なら従来の方式でバックエンドを呼ぶ | 旧と新 |
| 4 | 旧サインインの入口を `/auth/login` への 302 に置き換える。新規ログインは全員 Auth Server 経由になり、旧 Cookie を持つユーザーは期限まで旧経路で動く。問題があればこの 1 行を戻せば旧に復帰する | 旧と新。新規は新のみ |
| 5 | 旧 Cookie の寿命が過ぎたら、middleware の旧判定、旧 Token 系 Route Handler、旧ログイン画面、`domain` 属性付き Cookie を削除する | 新のみ |

段階 1 で MFA とパスワード再設定まで揃えておくこと。揃わないまま段階 4 に進むと、MFA が必要なユーザーがログインできなくなる。段階 1 と 4 だけが順序に依存し、それ以外の各段階は単独で戻せる。
