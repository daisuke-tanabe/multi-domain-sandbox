...
| `packages/api-core/src/usecases/resolve-tenant-context.ts` | API 側の Host → aud 確認、Token 検証、user / tenant / membership の 1 回の JOIN による認可。crm-api / cms-api が共有する。`auth/middleware.ts` は結果を HTTP に写像するだけ |
