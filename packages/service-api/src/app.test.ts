...
describe("authentication", () => {
  ...
  test("rejects a crm access token presented to the cms api", async () => {
    // aud はサービスごとに固定。サービスをまたいだ Token の持ち回りはできない
    const cms = await createApiHarness({
  ...
  test("accepts a cms access token on the cms api", async () => {
    const cms = await createApiHarness({
      signingKey: harness.signingKey,
      clock: harness.clock,
      audience: CMS_AUDIENCE,
  ...
  test("returns 404 when the request arrives at a host other than API_HOST", async () => {
    const token = await issueTestAccessToken(harness, { userId: ALICE_ID, tenantId: TANAKA_ID });
