...
import { api, apiVoid, describeError } from "./api.ts";
...
export function MembersPage({ members }: { members: ReadonlyArray<Member> }) {
  ...
  const invite = async (event: FormEvent<HTMLFormElement>) => {
    ...
    await run(() => api(inviteMemberResponseSchema, "/v1/members", { method: "POST", json: body }));
  ...
  const open = async (userId: string) => {
    setError(undefined);
    try {
      setSelected(await api(memberDetailResponseSchema, `/v1/members/${userId}`));
    } catch (cause: unknown) {
      setError(describeError(cause));
    }
  };
              ...
              <td>
                {permissions.has("members:manage") ? (
                  <select
                    ...
                    onChange={(event) =>
                      void run(() =>
                        api(memberResponseSchema, `/v1/members/${member.user_id}`, {
                          method: "PATCH",
              ...
              <td>
                ...
                {permissions.has("members:manage") && member.user_id !== me.user.id && (
                  <button
                    type="button"
                    onClick={() =>
                      void run(() => apiVoid(`/v1/members/${member.user_id}`, { method: "DELETE" }))
                    }
                  >
                    削除
                  </button>
        ...
        <PermissionEditor
          ...
          onSave={async (overrides) => {
            await run(() =>
              api(
                permissionOverridesResponseSchema,
                `/v1/members/${selected.member.user_id}/permissions`,
                {
                  method: "PUT",
                  json: { overrides },
                },
